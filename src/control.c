/*
  ...
  This code processes commands from the Previous control socket
*/
const char Control_fileid[] = "Previous control.c : " __DATE__ " " __TIME__;
 ...
 * 
 * This can be used by external Previous UI(s) for
 ...
 * 
 * This can be used by external Previous UI(s) on devices which input
...
/*-----------------------------------------------------------------------*/
/**
 * Show Previous remote usage info and return false
 */
static bool Control_Usage(const char *cmd)
{
	fprintf(stderr, "ERROR: unrecognized previous command: '%s'", cmd);
	fprintf(stderr,
		"Supported commands are:\n"
		"- previous-debug <Debug UI command>\n"
		"- previous-event <event to simulate>\n"
		"- previous-option <command line options>\n"
		"- previous-enable/disable/toggle <device name>\n"
		"- previous-path <config name> <new path>\n"
		"- previous-shortcut <shortcut name>\n"
		"- previous-embed-info\n"
		"- previous-stop\n"
		"- previous-cont\n"
		"The last two can be used to stop and continue the Previous emulation.\n"
...
/*-----------------------------------------------------------------------*/
/**
 * Parse Previous debug/event/option/toggle/path/shortcut command buffer.
...
void Control_ProcessBuffer(char *buffer)
{
	...
	do {
		...
		if (arg) {
			if (strcmp(cmd, "previous-option") == 0) {
				ok = Change_ApplyCommandline(arg);
			} else if (strcmp(cmd, "previous-debug") == 0) {
				ok = DebugUI_RemoteParse(arg);
			} else if (strcmp(cmd, "previous-shortcut") == 0) {
				ok = Shortcut_Invoke(arg);
			} else if (strcmp(cmd, "previous-event") == 0) {
				ok = Control_InsertEvent(arg);
			} else if (strcmp(cmd, "previous-path") == 0) {
				ok = Control_SetPath(arg);
			} else if (strcmp(cmd, "previous-enable") == 0) {
				ok = Control_DeviceAction(arg, DO_ENABLE);
			} else if (strcmp(cmd, "previous-disable") == 0) {
				ok = Control_DeviceAction(arg, DO_DISABLE);
			} else if (strcmp(cmd, "previous-toggle") == 0) {
				ok = Control_DeviceAction(arg, DO_TOGGLE);
			} else {
				ok = Control_Usage(cmd);
			}
		} else {
			if (strcmp(cmd, "previous-embed-info") == 0) {
				fprintf(stderr, "Embedded window ID change messages = ON\n");
				bSendEmbedInfo = true;
			} else if (strcmp(cmd, "previous-stop") == 0) {
				Main_PauseEmulation(true);
				bRemotePaused = true;
			} else if (strcmp(cmd, "previous-cont") == 0) {
...
/**
 * Reparent Previous window if so requested.  Needs to be done inside
 * Previous because if SDL itself is requested to reparent itself,
 ...
 * to be hidden) when SDL is asked to fullscreen, Previous window just
...
void Control_ReparentWindow(int width, int height, bool noembed)
{
	...
	if (noembed) {
		...
		/* hide WM window for Previous */
		XUnmapWindow(display, wm_win);
		/* reparent main Previous window to given parent */
...
void Control_ReparentWindow(int width, int height, bool noembed)
{
	...
	Log_Printf(LOG_TODO, "Support for Previous window reparenting not built in\n");
}
