...
static long nCurInstrCycPos;	/* Previous only : Stores where we have to patch in the current cycles value */
#endif
            ...
            switch (fetchmode)
        {
            ...
            if (cpu_level > 1) {
                ...
                if (!(flags & GF_AD8R)) {
                    ...
                    count_cycles_ea += 2;
#ifdef WINUAE_FOR_HATARI
                    /* Previous : on 68000 ST, Ad8r causes an unaligned memory prefetch and take 2 cycles more */
...
#ifdef WINUAE_FOR_HATARI
    /* Previous : Store the family of the instruction (used to check for pairing on ST,
    ...
    switch (curi->mnemo) {
...
#else
            /* Previous : use 2 cases to avoid 'tmp_newv' set but not used */
            if (cpu_level >= xBCD_KEEPS_NV_FLAGS) {
                printf ("\tint cflg;\n");
                printf ("\tif (newv_lo > 9) { newv_lo -= 6; }\n");
        ...
        case i_JSR:
            ...
            if (using_mmu) {
                ...
                if (curi->smode == Ad8r || curi->smode == PC8r) {
                    addcycles000 (6);
#ifdef WINUAE_FOR_HATARI
                    /* Previous : JSR in Ad8r and PC8r mode takes 22 cycles, but on ST it takes 24 cycles */
        ...
        case i_JMP:
            ...
            if (curi->smode == Ad8r || curi->smode == PC8r) {
                addcycles000 (6);
#ifdef WINUAE_FOR_HATARI
                /* Previous : JMP in Ad8r and PC8r mode takes 22 cycles, but on ST it takes 24 cycles */
                /* because of an unaligned memory prefetch in this EA mode */
                /* We add 2 cycles only in 68000 prefetch mode, 68000 CE mode is handled at the memory access level */
                if ( using_prefetch && !using_ce )
                    addcycles000 (2);
        ...
        case i_LEA:
            ...
            if (curi->smode == Ad8r || curi->smode == PC8r) {
                addcycles000 (2);
#ifdef WINUAE_FOR_HATARI
                /* Previous : LEA in Ad8r and PC8r mode takes 12 cycles, but on ST it takes 14 cycles */
                /* because of an unaligned memory prefetch in this EA mode */
                /* We add 2 cycles only in 68000 prefetch mode, 68000 CE mode is handled at the memory access level */
                if ( using_prefetch && !using_ce )
                    addcycles000 (2);
        ...
        case i_PEA:
            ...
            if (curi->smode == Ad8r || curi->smode == PC8r) {
                addcycles000 (2);
#ifdef WINUAE_FOR_HATARI
                /* Previous : PEA in Ad8r and PC8r mode takes 20 cycles, but on ST it takes 22 cycles */
                /* because of an unaligned memory prefetch in this EA mode */
                /* We add 2 cycles only in 68000 prefetch mode, 68000 CE mode is handled at the memory access level */
                if ( using_prefetch && !using_ce )
                    addcycles000 (2);
...
static void generate_includes (FILE * f, int id)
{
    ...
    fprintf (f, "#include \"previous-glue.h\"\n");
...
#ifdef WINUAE_FOR_HATARI
    /* Previous only : Now patch in the instruction cycles at the beginning of the function: */
    if (!using_ce020) {
