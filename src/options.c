/*
  ...
  Functions for showing and parsing all of Previous's command line options.
  
  To add a new option:
  - Add option ID to the enum
  - Add the option information to PreviousOptions[]
  - Add required actions for that ID to switch in Opt_ParseParameters()
*/
const char Options_fileid[] = "Previous options.c : " __DATE__ " " __TIME__;
...
static const opt_t PreviousOptions[] = {
	...
	{ OPT_CONFIRMQUIT, NULL, "--confirm-quit",
	  "<bool>", "Whether Previous confirms quit" },
	{ OPT_CONFIGFILE, "-c", "--configfile",
	  "<file>", "Use <file> instead of the default previous config file" },
	...
	{ OPT_SAVECONFIG, NULL, "--saveconfig",
	  NULL, "Save current Previous configuration and exit" },
	{ OPT_PARACHUTE, NULL, "--no-parachute",
	  NULL, "Disable SDL parachute to get Previous core dumps" },
#if HAVE_UNIX_DOMAIN_SOCKETS
	{ OPT_CONTROLSOCKET, NULL, "--control-socket",
	  "<file>", "Previous reads options from given socket at run-time" },
#endif
...
{
	const opt_t *opt = PreviousOptions;
...
/**
 * Show Previous version and usage.
...
static bool Opt_ShowError(unsigned int optid, const char *value, const char *error)
{
	...
	Opt_ShowVersion();
	printf("Usage:\n previous [options] [disk image name]\n\n"
	...
	if (error)
	{
		...
		{
			for (opt = PreviousOptions; opt->id != OPT_ERROR; opt++)
			{
				if (optid == opt->id)
					break;
			}
...
/**
 * matches string under given index in the argv against all Previous
 ...
 * to be disabled to get proper Previous core dumps.  By default returns
 * zero so that SDL parachute will be used to restore video mode on
 * unclean Previous termination.
 */
...
/**
 * parse all Previous command line options and set Previous state accordingly.
...
//			ConfigureParams.Sound.SdlAudioBufferSize = temp;
		...
		case OPT_SAVECONFIG:
			/* Previous-UI needs Previous config to start */
			Configuration_Save();
			exit(0);
			break;
		...
		default:
			return Opt_ShowError(OPT_ERROR, argv[i], "Internal Previous error, unhandled option");
		}
		if (!ok)
		{
...
char *Opt_MatchOption(const char *text, int state)
{
	...
	/* next match */
	while (i < ARRAYSIZE(PreviousOptions)) {
		name = PreviousOptions[i++].str;
