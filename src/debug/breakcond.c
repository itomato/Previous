/*
  ...
  for the syntax.
*/
const char BreakCond_fileid[] = "Previous breakcond.c : " __DATE__ " " __TIME__;
...
#include "video.h"	/* for Previous video variable addresses */
...
typedef enum {	
	...
	/* internal Previous value variables */
	VALUE_TYPE_VAR32      = 4,
...
typedef struct {
	...
	value_t valuetype;	/* Previous value variable type */
	union {
		Uint32 number;
		Uint16 (*func16)(void);
		Uint32 (*func32)(void);
...
/* Previous variable name & address array items */
typedef struct {
	const char *name;
	Uint32 *addr;
	value_t vtype;
...
char *BreakCond_MatchCpuVariable(const char *text, int state)
{
	...
	/* next match */
//	while (i < ARRAYSIZE(previous_vars)) {
//		name = previous_vars[i++].name;
...
/**
 * If given string is a Previous variable name, set bc_value
...
//	ENTERFUNC(("BreakCond_ParseVariable('%s')\n", name));
	/* bisect */
//	l = 0;
//	r = ARRAYSIZE(previous_vars) - 1;
//	do {
//		m = (l+r) >> 1;
//		dir = strcasecmp(name, previous_vars[m].name);
//		if (dir == 0) {
//			bc_value->value.reg32 = previous_vars[m].addr;
//			bc_value->valuetype = previous_vars[m].vtype;
//			bc_value->bits = previous_vars[m].bits;
...
/**
 * If given string is a Previous variable name, set value to given
 * variable value and return true, otherwise return false.
 */
bool BreakCond_GetPreviousVariable(const char *name, Uint32 *value)
{
    bc_value_t bc_value;
...
static bool BreakCond_ParseValue(parser_state_t *pstate, bc_value_t *bc_value)
{
	...
	if (isalpha(*str) || *str == '_') {
		...
		if (bc_value->is_indirect) {
			...
			/* a valid Previous variable or register name?
...
"\n"
"  Valid Previous variable names (and their current values) are:\n", stderr);
//	for (i = 0; i < ARRAYSIZE(previous_vars); i++) {
//		switch (previous_vars[i].vtype) {
//		case VALUE_TYPE_FUNCTION32:
//			value = ((Uint32(*)(void))(previous_vars[i].addr))();
//			break;
//		case VALUE_TYPE_VAR32:
//			value = *(previous_vars[i].addr);
//			break;
//		default:
//			fprintf(stderr, "ERROR: variable '%s' has unsupported type '%d'\n",
//				previous_vars[i].name, previous_vars[i].vtype);
//			continue;
//		}
//		fprintf(stderr, "  - %s ($%x)", previous_vars[i].name, value);
//		if (previous_vars[i].constraints) {
//			fprintf(stderr, ", %s\n", previous_vars[i].constraints);
//		} else {
//			fprintf(stderr, "\n");
//		}
//	}
