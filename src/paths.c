...
*/
const char Paths_fileid[] = "Previous paths.c : " __DATE__ " " __TIME__;
...
static char sDataDir[FILENAME_MAX];       /* Directory where data files of Previous can be found */
static char sUserHomeDir[FILENAME_MAX];   /* User's home directory ($HOME) */
static char sPreviousHomeDir[FILENAME_MAX]; /* Previous's home directory ($HOME/.previous/) */
...
/**
 * Return pointer to Previous's home directory string
 */
const char *Paths_GetPreviousHome(void)
{
	return sPreviousHomeDir;
}
...
/**
 * Locate the directory where the previous executable resides
 */
static char *Paths_InitExecDir(const char *argv0)
{
	char *psExecDir;  /* Path string where the previous executable can be found */
 ...
 * and Previous's home directory (~/.previous)
 */
static void Paths_InitHomeDirs(void)
{
	...
	if (!psHome)
	{
		...
		strcpy(sPreviousHomeDir, sWorkingDir);
	}
	else
	{
		...
		snprintf(sPreviousHomeDir, FILENAME_MAX, "%s%c.previous",
		         sUserHomeDir, PATHSEP);
		if (!File_DirExists(sPreviousHomeDir))
		{
			/* Previous home directory does not exists yet...
			 * ...so let's try to create it: */
			if (mkdir(sPreviousHomeDir, 0755) != 0)
			{
				/* Failed to create, so use user's home dir instead */
				strcpy(sPreviousHomeDir, sUserHomeDir);
			}
		}
	}
}
...
void Paths_Init(const char *argv0)
{
	...
	/* fprintf(stderr, " WorkingDir = %s\n DataDir = %s\n UserHomeDir = %s\n PreviousHomeDir = %s\n",
	        sWorkingDir, sDataDir, sUserHomeDir, sPreviousHomeDir); */
}
