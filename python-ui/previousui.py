#!/usr/bin/env python
#
# A PyGtk UI for Previous that can embed the Previous emulator window.
#
...
from debugui import PreviousDebugUI
from previous import Previous, PreviousConfigMapping
from uihelpers import UInfo, UIHelp, create_button, create_toolbutton, \
     create_toggle, PreviousTextInsert, get_open_filename, get_save_filename
...
# ---------------------------------------------------------------
# Class with Previous and configuration instances which methods are
# called to change those (with additional dialogs or directly).
# Owns the application window and socket widget embedding Previous.
class UICallbacks:
    tmpconfpath = os.path.expanduser("~/.previous/.tmp.cfg")
    def __init__(self):
        # Previous and configuration
        self.previous = Previous()
        error = self.previous.is_compatible()
        if error:
            ErrorDialog(None).run(error)
            sys.exit(-1)
            
        self.config = PreviousConfigMapping(self.previous)
        try:
            self.config.validate()
        except (KeyError, AttributeError):
            NoteDialog(None).run("Loading Previous configuration failed!\nRetrying after saving Previous configuration.")
            self.previous.save_config()
            self.config = PreviousConfigMapping(self.previous)
            self.config.validate()
        
        # windows are created when needed
        self.mainwin = None
        self.previouswin = None
        self.debugui = None
        self.panels = {}
        ...
        # TODO: Previous UI own configuration settings save/load
    ...
    def _add_uisocket(self, box):
        # add Previous parent container to given box
        socket = gtk.Socket()
        # without this, closing Previous would remove the socket widget
        ...
        # set initial embedded previous size
        width, height = self.config.get_window_size()
        socket.set_size_request(width, height)
        # no resizing for the Previous window
        box.pack_start(socket, False, False)
        self.previouswin = socket

    # ------- run callback -----------
    def _socket_cb(self, fd, event):
        if event != gobject.IO_IN:
            # previous process died, make sure Previous instance notices
            self.previous.kill()
            return False
        width, height = self.previous.get_embed_info()
        print("New size = %d x %d" % (width, height))
        oldwidth, oldheight = self.previouswin.get_size_request()
        self.previouswin.set_size_request(width, height)
    ...
    def run(self, widget = None):
        if not self.killdialog.run(self.previous):
            return
        if self.io_id:
            gobject.source_remove(self.io_id)
        args = ["--configfile"]
        # whether to use Previous config or unsaved Previous UI config?
        ...
        if self.floppy:
            args += self.floppy
        if self.previouswin:
            size = self.previouswin.window.get_size()
            self.previous.run(args, self.previouswin.window)
            # get notifications of Previous window size changes
            self.previous.enable_embed_info()
            socket = self.previous.get_control_socket().fileno()
            events = gobject.IO_IN | gobject.IO_HUP | gobject.IO_ERR
            self.io_id = gobject.io_add_watch(socket, events, self._socket_cb)
            # all keyboard events should go to Previous window
            self.previouswin.grab_focus()
        else:
            self.previous.run(args)
    ...
    def reset(self, widget):
        ...
        self.resetdialog.run(self.previous)
    ...
    def inputs(self, widget):
        ...
        self.inputdialog.run(self.previous)
    ...
    def machine(self, widget):
        ...
        if self.machinedialog.run(self.config):
            self.previous.trigger_shortcut("coldreset")
    ...
    def debugger(self, widget):
        if not self.debugui:
            self.debugui = PreviousDebugUI(self.previous)
        self.debugui.show()
    ...
    def trace(self, widget):
        ...
        self.tracepoints = self.tracedialog.run(self.previous, self.tracepoints)

    # ------ snapshot load/save callbacks ---------
    def load(self, widget):
        path = os.path.expanduser("~/.previous/previous.sav")
    ...
    def save(self, widget):
        self.previous.trigger_shortcut("savemem")
    ...
    def config_load(self, widget):
        ...
        if filename:
            self.previous.change_option("--configfile %s" % filename)
    ...
    def set_fullscreen(self, widget):
        # if user can select this, Previous isn't in fullscreen
        self.previous.change_option("--fullscreen")

    # ------- screenshot callback -----------
    def screenshot(self, widget):
        self.previous.trigger_shortcut("screenshot")

    # ------- record callbacks -----------
    def recanim(self, widget):
        self.previous.trigger_shortcut("recanim")

    def recsound(self, widget):
        self.previous.trigger_shortcut("recsound")

    # ------- insert key special callback -----------
    def keypress(self, widget, code):
        self.previous.insert_event("keypress %s" % code)

    def textinsert(self, widget, text):
        PreviousTextInsert(self.previous, text)
...
class UIActions:
    ...
    def _get_menu(self):
        ...
        ("Debug", ("debug", "trace")),
        ("Help", ("manual", "compatibility", "release", "todo", None, "bugs", "mails", "changes", None, "authors", "previous", "previousui", "about",))
        )
        bar = gtk.MenuBar()
    ...
    def run(self, floppy, havemenu, fullscreen, embed):
        ...
        # create menu?
        if havemenu:
            # this would steal keys from embedded Previous
            if not embed:
        ...
        # ugly, Previous socket window ID can be gotten only
...
def usage(actions, msg=None):
    ...
    print("\t-e, --embed\t\tembed Previous window in middle of controls")
