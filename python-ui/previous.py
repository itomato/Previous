#!/usr/bin/env python
#
# Classes for Previous emulator instance and mapping its congfiguration
...
class Previous:
    "running previous instance and methods for communicating with it"
    basepath = "/tmp/previous-ui-" + str(os.getpid())
    ...
    def __init__(self, previousbin = None):
        # collect previous process zombies without waitpid()
        signal.signal(signal.SIGCHLD, signal.SIG_IGN)
        if previousbin:
            self.previousbin = previousbin
        else:
            self.previousbin = "previous"
        self._create_server()
        self.control = None
        self.paused = False
        self.pid = 0
    ...
    def is_compatible(self):
        "check Previous compatibility and return error string if it's not"
        for line in os.popen(self.previousbin + " -h").readlines():
            if line.find("--control-socket") >= 0:
                return None
        return "Previous not found or it doesn't support the required --control-socket option!"

    def save_config(self):
        os.popen(self.previousbin + " --saveconfig")
    ...
    def _send_message(self, msg):
        if self.control:
            ...
            print("ERROR: no Previous (control socket)")
            return False
        
    def change_option(self, option):
        "change_option(option), changes given Previous cli option"
        return self._send_message("previous-option %s\n" % option)
        
    def set_path(self, key, path):
        "set_path(key, path), sets path with given key"
        return self._send_message("previous-path %s %s\n" % (key, path))
    ...
    def set_device(self, device, enabled):
        ...
        if enabled:
            return self._send_message("previous-enable %s\n" % device)
        else:
            return self._send_message("previous-disable %s\n" % device)
        
    def trigger_shortcut(self, shortcut):
        "trigger_shortcut(shortcut), triggers given Previous (keyboard) shortcut"
        return self._send_message("previous-shortcut %s\n" % shortcut)

    def insert_event(self, event):
        "insert_event(event), synthetizes given key/mouse Atari event"
        return self._send_message("previous-event %s\n" % event)

    def debug_command(self, cmd):
        "debug_command(command), runs given Previous debugger command"
        return self._send_message("previous-debug %s\n" % cmd)

    def pause(self):
        "pause(), pauses Previous emulation"
        return self._send_message("previous-stop\n")

    def unpause(self):
        "unpause(), continues Previous emulation"
        return self._send_message("previous-cont\n")
    
    def _open_output_file(self, previouscommand, option, path):
        ...
        # ask Previous to open/create the requested output file...
        previouscommand("%s %s" % (option, path))
        wait = 0.025
    ...
    def enable_embed_info(self):
        "enable_embed_info(), request embedded Previous window ID change information"
        self._send_message("previous-embed-info\n")

    def get_embed_info(self):
        "get_embed_info() -> (width, height), get embedded Previous window size"
    ...
    def is_running(self):
        "is_running() -> bool, True if Previous is running, False otherwise"
        if not self.pid:
            return False
        try:
            os.waitpid(self.pid, os.WNOHANG)
        except OSError as value:
            print("Previous PID %d had exited in the meanwhile:\n\t%s" % (self.pid, value))
            self.pid = 0
            return False
        return True
    
    def run(self, extra_args = None, parent_win = None):
        "run([parent window][,embedding args]), runs Previous"
        # if parent_win given, embed Previous to it
        pid = os.fork()
        if pid < 0:
        ...
        if pid:
            ...
            self.pid = pid
            if self.server:
                print("WAIT previous to connect to control socket...")
            ...
            args = [self.previousbin, "--confirm-quit", "off"]
            if self.server:
                args += ["--control-socket", self.controlpath]
            if extra_args:
                args += extra_args
            print("RUN:", args)
            os.execvpe(self.previousbin, args, env)
    ...
    def _set_embed_env(self, env, parent_win):
        ...
        #
        # Instead we tell previous to reparent itself after creating
        # its own window into this program widget window
        env["PARENT_WIN_ID"] = str(win_id)

    def kill(self):
        "kill(), kill Previous if it's running"
        if self.pid:
            os.kill(self.pid, signal.SIGKILL)
            print("killed previous with PID %d" % self.pid)
            self.pid = 0
        if self.control:
            self.control.close()
            self.control = None
...
# Mapping of requested values both to Previous configuration
...
# don't match to current Previous.  So before using this the current
# Previous configuration should have been saved at least once.
#
...
class PreviousConfigMapping(ConfigStore):
    _paths = {
    ...
    "access methods to Previous configuration file variables and command line options"
    ...
    def _change_option(self, option):
        if self._lock_updates:
            ...
            self._previous.change_option(option)

    def lock_updates(self):
        "lock_updates(), collect Previous configuration changes"
        self._lock_updates = True
    
    def flush_updates(self):
        "flush_updates(), apply collected Previous configuration changes"
        self._lock_updates = False
        if not self._options:
            return
        self._previous.change_option(" ".join(self._options))
        self._options = []
    ...
    def set_paths(self, paths):
        for key, path in paths:
            ...
            self._previous.set_path(key, path)
    ...
    def set_midi(self, value):
        ...
        self._previous.set_device("midi", value)
    ...
    def set_printer(self, value):
        ...
        self._previous.set_device("printer", value)
    ...
    def set_rs232(self, value):
        ...
        self._previous.set_device("rs232", value)
    ...
    def set_sound(self, enabled, hz):
        # map get_sound_values() index to Previous config
        try:
            hz = int(hz)
        except ValueError:
            hz = 0
    ...
    # ------------ configured Previous window size ---------------
    def get_window_size(self):
        if self.get("[Screen]", "bFullScreen"):
            print("WARNING: don't start Previous UI with fullscreened Previous!")
