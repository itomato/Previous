#!/usr/bin/env python
#
# Misc common helper classes and functions for the Previous UI
#
...
class UIHelp:
    ...
    def get_doc_path(self):
        """return path or URL to Previous docs or None"""
        # first try whether there are local Previous docs in standard place
        # for this Previous/UI version
        sep = os.sep
        path = self.get_binary_path("previous")
        path = sep.join(path.split(sep)[:-2]) # remove "bin/previous"
        path = path + sep + "share" + sep + "doc" + sep + "previous" + sep
        if os.path.exists(path + "manual.html"):
            return path
        # if not, point to latest Previous HG version docs
        print("WARNING: Previous manual not found at:", path + "manual.html")
        return "http://hg.berlios.de/repos/previous/raw-file/tip/doc/"
    ...
    def view_previous_manual(self, dummy=None):
        self.view_url(self._path + "manual.html", "Previous manual")

    def view_previous_compatibility(self, dummy=None):
        self.view_url(self._path + "compatibility.html", "Previous compatibility list")

    def view_previous_releasenotes(self, dummy=None):
        self.view_url(self._path + "release-notes.txt", "Previous release notes")

    def view_previous_todo(self, dummy=None):
        self.view_url(self._path + "todo.txt", "Previous TODO items")

    def view_previous_bugs(self, dummy=None):
        self.view_url("http://developer.berlios.de/bugs/?group_id=10436", "Previous bug tracker")

    def view_previous_mails(self, dummy=None):
        self.view_url("http://developer.berlios.de/mail/?group_id=10436", "Previous mailing lists")

    def view_previous_repository(self, dummy=None):
        self.view_url("http://hg.berlios.de/repos/previous", "latest Previous changes")

    def view_previous_authors(self, dummy=None):
        self.view_url(self._path + "authors.txt", "Previous authors")

    def view_previous_page(self, dummy=None):
        self.view_url("http://previous.berlios.de/", "Previous home page")

    def view_previousui_page(self, dummy=None):
        self.view_url("http://koti.mbnet.fi/tammat/previous/previous-ui.shtml", "Previous UI home page")


# --------------------------------------------------------
# auxiliary class+callback to be used with the PasteDialog

class PreviousTextInsert:
...
# callback to insert text object to Previous character at the time
...
def _text_insert_cb(textobj):
    ...
    if textobj.pressed:
        ...
        textobj.previous.insert_event("keyup %s" % char)
        textobj.index += 1
        ...
        textobj.previous.insert_event("keydown %s" % char)
    # call again
    return True
