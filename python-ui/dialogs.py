#!/usr/bin/env python
#
# Classes for the Previous UI dialogs
#
...
from uihelpers import UInfo, PreviousTextInsert, create_table_dialog, \
...
class NoteDialog(PreviousUIDialog):
...
class AskDialog(PreviousUIDialog):
    def run(self, text):
...
class AboutDialog(PreviousUIDialog):
    def __init__(self, parent):
        dialog = gtk.AboutDialog()
        dialog.set_transient_for(parent)
        dialog.set_name(UInfo.name)
        dialog.set_version(UInfo.version)
        dialog.set_website("http://previous.berlios.de/")
        dialog.set_website_label("Previous emulator www-site")
        dialog.set_authors(["Eero Tamminen"])
        dialog.set_artists(["The logo is from Previous"])
...
class InputDialog(PreviousUIDialog):
    def __init__(self, parent):
        ...
        insert.set_tooltip_text("Insert given text to Previous window")
    ...
    def _enter_cb(self, widget):
        self.previous.insert_event("keypress 28") # Enter key scancode

    def _doubleclick_cb(self, widget):
        self.previous.insert_event("doubleclick")

    def _rightpress_cb(self, widget):
        self.previous.insert_event("rightdown")

    def _rightrelease_cb(self, widget):
        self.previous.insert_event("rightup")

    def run(self, previous):
        "run(previous), do text/mouse click input"
...
class QuitSaveDialog(PreviousUIDialog):
    def __init__(self, parent):
...
class KillDialog(PreviousUIDialog):
    def __init__(self, parent):
        self.dialog = gtk.MessageDialog(parent,
        gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
        gtk.MESSAGE_QUESTION, gtk.BUTTONS_OK_CANCEL,
        """\
Previous emulator is already/still running and it needs to be terminated first. However, if it contains unsaved data, that will be lost.

Terminate Previous anyway?""")

    def run(self, previous):
        "run(previous) -> True if Previous killed, False if left running"
        if not previous.is_running():
            return True
        # Previous is running, OK to kill?
        response = self.dialog.run()
        self.dialog.hide()
...
class ResetDialog(PreviousUIDialog):
    COLD = 1
    WARM = 2
    def __init__(self, parent):
    ...
    def run(self, previous):
        "run(previous) -> True if Previous rebooted, False if canceled"
        if not previous.is_running():
            return False
        # Previous is running, how to reboot?
        response = self.dialog.run()
        self.dialog.hide()
        if response == self.COLD:
            previous.trigger_shortcut("coldreset")
        elif response == self.WARM:
            previous.trigger_shortcut("warmreset")
        else:
            return False
        return True
...
class FloppyDialog(PreviousUIDialog):
...
class HardDiskDialog(PreviousUIDialog):
    def _create_dialog(self, config):
...
class DisplayDialog(PreviousUIDialog):
...
class JoystickDialog(PreviousUIDialog):
    def _create_dialog(self, config):
...
class PeripheralDialog(PreviousUIDialog):
    def _create_dialog(self, config):
...
class PathDialog(PreviousUIDialog):
    def _create_dialog(self, config):
...
class SoundDialog(PreviousUIDialog):
...
class TraceDialog(PreviousUIDialog):
    # you can get this list with:
    # previous --trace help 2>&1|awk '/all$/{next} /^  [^-]/ {printf("\"%s\",\n", $1)}'
    tracepoints = [
        "video_sync",
        "video_res",
        "video_color",
    ...
    def run(self, previous, savedpoints):
        "run(previous,tracepoints) -> tracepoints, caller saves tracepoints"
        self.savedpoints = savedpoints
        while 1:
            response = self.dialog.run()
            if response == gtk.RESPONSE_APPLY:
                previous.change_option("--trace %s" % self._get_traces())
            else:
                self.dialog.hide()
...
class MachineDialog(PreviousUIDialog):
