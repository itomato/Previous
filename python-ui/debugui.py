#!/usr/bin/env python
#
# A Debug UI for the Previous, part of PyGtk Previous UI
#
...
class LoadDialog:
    def __init__(self, parent):
        ...
        chooser.set_local_only(True)  # Previous cannot access URIs
...
class MemoryAddress:
    ...
    def __init__(self, previousobj):
        # previous
        self.debug_output = previousobj.open_debug_output()
        self.previous = previousobj
        # widgets
    ...
    def _get_registers(self):
        self.previous.debug_command("r")
        output = self.previous.get_lines(self.debug_output)
        if not self.first:
    ...
    def _get_memdump(self, address, move_idx):
        ...
        self.previous.debug_command("m $%06x-$%06x" % (self.first, self.last))
    ...
    def _get_disasm(self, address, move_idx):
        ...
        if move_idx < 0:
            ...
            if move_idx == Constants.MOVE_MED and self.second:
                ...
                address += offset
        self._set_clamped(address, address+screenful)
        self.previous.debug_command("d $%06x-$%06x" % (self.first, self.last))
        # get & set debugger command results
        output = self.previous.get_lines(self.debug_output)
...
# the Previous debugger UI class and methods
class PreviousDebugUI:
    
    def __init__(self, previousobj, do_destroy = False):
        self.address = MemoryAddress(previousobj)
        self.previous = previousobj
        ...
        self.window = self.create_ui("Previous Debug UI", do_destroy)
    ...
    def memload_cb(self, widget):
        ...
        if filename and address:
            self.previous.debug_command("l %s $%06x" % (filename, address))
    ...
    def memsave_cb(self, widget):
        ...
        if filename and address and length:
            self.previous.debug_command("s %s $%06x $%06x" % (filename, address, length))
    ...
    def quit(self, widget, arg):
        KillDialog(self.window).run(self.previous)
        gtk.main_quit()
...
def main():
    ...
    previousobj = Previous()
    if len(sys.argv) > 1:
        if sys.argv[1] in ("-h", "--help"):
            print("usage: %s [previous options]" % os.path.basename(sys.argv[0]))
            return
        args = sys.argv[1:]
    else:
        args = None
    previousobj.run(args)

    info = UInfo()
    debugui = PreviousDebugUI(previousobj, True)
